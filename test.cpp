...
void showTasks(std::unique_ptr<sql::Connection> &conn) {
    try {
        ...
        // Execute query
        sql::ResultSet *res = stmnt->executeQuery("select * from astra.appointments");
...
int main(int argc, char **argv){
    if (argc==1){
        ...
        try {
            ...
            sql::SQLString url("jdbc:mariadb://astra.mysql.uhserver.com:3306");
            sql::Properties properties({{"user", "pedroastra"}, {"password", "E2,71828e"}});
